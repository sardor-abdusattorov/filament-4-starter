    ...
    <body class="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 min-h-screen flex flex-col">
        ...
        <main class="flex-1 flex items-center justify-center px-6">
            <div class="text-center max-w-2xl">
                ...
                <h1 class="text-4xl md:text-5xl font-bold text-slate-800 dark:text-white mb-4">
                    {{ __('app.label.welcome') }}
                </h1>

                <p class="text-lg text-slate-600 dark:text-slate-300 mb-8">
                    {{ __('app.label.welcome_description') }}
                </p>
                    ...
                    @auth
                        <a href="{{ url('/admin') }}"
                           class="px-6 py-3 text-base font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-md hover:shadow-lg">
                            {{ __('app.label.go_to_dashboard') }}
                        </a>
                    @else
                        <a href="{{ route('filament.admin.auth.login') }}"
                           class="px-6 py-3 text-base font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-md hover:shadow-lg">
                            {{ __('app.label.get_started') }}
                        </a>
                    @endauth
                </div>
                ...
                <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div class="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
                        ...
                        <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-2">{{ __('app.label.feature_secure') }}</h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('app.label.feature_secure_desc') }}</p>
                    </div>
                    ...
                    <div class="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
                        ...
                        <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-2">{{ __('app.label.feature_fast') }}</h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('app.label.feature_fast_desc') }}</p>
                    </div>
                    ...
                    <div class="p-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm">
                        ...
                        <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-2">{{ __('app.label.feature_multilang') }}</h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('app.label.feature_multilang_desc') }}</p>
                    </div>
                </div>
            </div>
        </main>
        ...
        <footer class="py-6 px-6">
            <div class="max-w-6xl mx-auto text-center text-sm text-slate-500 dark:text-slate-400">
                &copy; {{ date('Y') }} {{ config('app.name', 'Laravel') }}. {{ __('app.label.all_rights_reserved') }}
            </div>
        </footer>
    </body>
</html>
