<div x-data="{ open: @entangle('isOpen') }">
    ...
    >
        ...
        >
            {{-- Header --}}
            <div class="sticky top-0 z-10 flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Settings') }}</h2>
            ...
            <div class="p-4 space-y-6">
                {{-- Theme --}}
                <div>
                    <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Mode') }}</label>
                        ...
                        @foreach(['light' => 'heroicon-o-sun', 'dark' => 'heroicon-o-moon', 'system' => 'heroicon-o-computer-desktop'] as $mode => $icon)
                            <button
                                ...
                                @class([
                                    ...
                                    'hover:bg-gray-50 dark:hover:bg-gray-700' => $theme !== $mode,
                                ])
                            >
                ...
                {{-- Layout --}}
                <div>
                    <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Layout') }}</label>
                    <div class="mt-2 grid grid-cols-2 gap-2">
                        @foreach(['sidebar' => 'Sidebar', 'topbar' => 'Topbar'] as $layoutKey => $layoutLabel)
                            <button
                            ...
                            >
                                ...
                                @endif
                                <span class="mt-1 text-xs text-gray-600 dark:text-gray-400">{{ $layoutLabel }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
                ...
                {{-- Color --}}
                <div>
                    <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Color') }}</label>
                    <div class="mt-2 flex flex-wrap gap-2">
                        ...
                        @foreach($colors as $color)
                            <button
                                ...
                                type="button"
                                class="relative w-7 h-7 rounded-full transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900"
                                style="background-color: {{ $colorHex[$color] ?? '#3b82f6' }}"
                                title="{{ ucfirst($color) }}"
                            >
                ...
                {{-- Font --}}
                <div>
                    <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Font') }}</label>
                    <div class="mt-2 grid grid-cols-2 gap-2">
                        @foreach($fonts as $font)
                            <button
                                ...
                                type="button"
                                @class([
                                    'flex flex-col items-center p-2 border-2 rounded-lg transition',
                                    'border-primary-500 bg-primary-50 dark:bg-primary-950' => $fontFamily === $font,
                                    'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600' => $fontFamily !== $font,
                                ])
                                style="font-family: '{{ $font }}', sans-serif"
                            >
                                <span class="text-xl font-medium text-gray-700 dark:text-gray-300">Aa</span>
                                <span class="text-[10px] text-gray-500 dark:text-gray-400">{{ $font }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between">
                        <label class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{{ __('Font Size') }}</label>
                    ...
                    <input
                        ...
                        class="mt-2 w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700 accent-primary-500"
                    />
                </div>
                ...
                <button
                    wire:click="resetSettings"
                    type="button"
                    class="w-full py-2 px-4 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition"
                >
