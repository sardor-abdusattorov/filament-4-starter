<div x-data="{ open: @entangle('isOpen') }">
    ...
    >
        ...
        <div
            ...
            class="fixed inset-y-0 right-0 w-full max-w-xs bg-white dark:bg-gray-900 shadow-xl overflow-y-auto"
        >
            {{-- Header --}}
                ...
                <div>
                    ...
                    <div class="mt-2 grid grid-cols-2 gap-2">
                        @foreach(['sidebar' => 'Sidebar', 'topbar' => 'Topbar'] as $key => $label)
                            ...
                            >
                                @if($key === 'sidebar')
                                    <div class="flex w-10 h-7 border border-gray-300 dark:border-gray-600 rounded overflow-hidden">
                                        <div class="w-2.5 bg-gray-400 dark:bg-gray-500"></div>
                                        <div class="flex-1 bg-gray-100 dark:bg-gray-800"></div>
                                    </div>
                                @else
                                    <div class="flex flex-col w-10 h-7 border border-gray-300 dark:border-gray-600 rounded overflow-hidden">
                                        <div class="h-1.5 bg-gray-400 dark:bg-gray-500"></div>
                                        <div class="flex-1 bg-gray-100 dark:bg-gray-800"></div>
                                    </div>
                                @endif
                                <span class="mt-1 text-[10px] text-gray-600 dark:text-gray-400">{{ $label }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
                    ...
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        ...
                        @foreach($colors as $color)
                            <button
                                ...
                                type="button"
                                class="relative w-6 h-6 rounded-full hover:scale-110 transition-transform"
                                style="background-color: {{ $hex[$color] ?? '#3b82f6' }}"
                                title="{{ ucfirst($color) }}"
                            >
                                @if($primaryColor === $color)
                                    <x-filament::icon icon="heroicon-s-check" class="absolute inset-0 m-auto w-3.5 h-3.5 text-white" />
                                @endif
                            </button>
                        @endforeach
                    </div>
                    ...
                    <div class="mt-2 grid grid-cols-3 gap-1.5">
                        @foreach($fonts as $font)
                            <button
                                ...
                                type="button"
                                @class([
                                    'flex flex-col items-center p-1.5 border-2 rounded-lg transition',
                            ...
                            >
                                ...
                                <span class="text-[8px] text-gray-500 dark:text-gray-400 truncate w-full text-center">{{ $font }}</span>
                            </button>
                        @endforeach
                    </div>
                </div>
