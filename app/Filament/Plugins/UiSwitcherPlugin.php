...
{
    protected static array $colorPalettes = [
    ...
    {
        // Register custom styles for primary color in the head
        FilamentView::registerRenderHook(
            PanelsRenderHook::HEAD_END,
            fn (): string => $this->getFontsLink() . $this->getColorStyles(),
        );
    ...
    protected function getColorStyles(): string
    {
        ...
        if (! isset(self::$colorPalettes[$colorName])) {
            return '';
        }

        $palette = self::$colorPalettes[$colorName];
        $cssVars = '';

        foreach ($palette as $shade => $rgb) {
            $cssVars .= "--primary-{$shade}: {$rgb};\n";
        }
